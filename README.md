...
The `part_classification` section reports `sheet_metal`, `non_sheet_metal`, or `unknown`, together with confidence and a concise geometric reason. A `non_sheet_metal` result keeps CAD geometry available but blocks the normal 2D laser/bending quote: `quote_applicability.status` is `not_applicable`, the sheet-metal process plan is empty, and sheet-metal times and totals are not fabricated.
