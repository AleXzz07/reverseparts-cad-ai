...
Circular holes expose circumference and area in addition to diameter and radius. Slots expose unambiguous `overall_length_mm`, `straight_length_mm`, `width_mm`, `end_radius_mm`, `perimeter_mm`, and `area_mm2`; the historical `length_mm` contour value remains only for dataset/API compatibility. Rounded rectangular openings are classified separately and expose `overall_length_mm`, `width_mm`, `corner_radius_mm`, `perimeter_mm`, and `area_mm2`. Polygonal openings use `perimeter_mm` for contour length and `max_dimension_mm` for the largest geometric bounding-box dimension. Historical payloads that stored the polygon perimeter in `max_dimension_mm` remain readable by the cutting-length fallback, but all newly generated analyses use the corrected semantics. Polygonal, formed, and unknown planar openings expose perimeter and area when FreeCAD can build a reliable planar face.
